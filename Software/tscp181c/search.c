...
void think(int output, int messages)
{
	int i, j, x;
	...
	stop_search = FALSE;
	setjmp(env); // setjmp returns a value but we don't really care
	             // for this functionality. longjmp's 2nd argument is
	             // what setjmp returns. Ex: x = setjmp(env) at first 
	             // x will be assigned 0. When longjmp is used then we
	             // jump back to the setjmp line and this time x = the
	             // value in the 2nd argument of longjmp.
	if (stop_search) {
	...
	if (output == 1)
		if (messages){printf("ply      nodes  score  pv\n");}
	for (i = 1; i <= max_depth; ++i) {
		follow_pv = TRUE;
		x = search(-10000, 10000, i);
		if (output == 1)
			if (messages){printf("%3d  %9d  %5d ", i, nodes, x);}
		else if (output == 2)
			if (messages){printf("%d %d %d %d", i, x, (get_ms() - start_time) / 10, nodes);}
		if (output) {
			for (j = 0; j < pv_length[0]; ++j)
				if (messages){printf(" %s", move_str(pv[0][j].b));}
			if (messages){printf("\n");}
			fflush(stdout);
		}
		if (x > 9000 || x < -9000)

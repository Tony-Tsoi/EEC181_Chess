...
/* gen() generates pseudo-legal moves for the current position.
   ...
   stack." */
// To my knowledge, the "move stack" is a list of all current 
// pseudo-legal moves.
void gen()
{
	int i, j, n;
	...
	send_board_state(); // SW to HW
	
	SW_to_HW_Timer = *curval;
	
	Main_Pointer = (int*) get_virtual_addr(0xC0000000, 0x0, 0);
	*Main_Pointer = *Main_Pointer | 0x00000001; //  Start = 1
	
	Main_Pointer = (int*) get_virtual_addr(0xC0000000, 0x0, 0);
	while ( (*Main_Pointer & 0b10) == 0b00 )
	{;} // while(done == 0)
	
	*Main_Pointer = *Main_Pointer & (~0x00000001); // Start = 0
	
	HW_to_SW_Timer = *curval;

	Main_Pointer = (int*) get_virtual_addr(0xC0000000, 0x0, 16); // Read header
	TotalMoves = *Main_Pointer;
	//if (side == LIGHT){printf("Side: WHITE\n");}
	//else {printf("Side: BLACK\n");}
	//printf("TotalMoves = %i\n", TotalMoves);
	
	for (i = 0; i < TotalMoves; ++i) // HW to SW
	{
		Main_Pointer = (int*) get_virtual_addr(0xC0000000, 0x0, 17+i);
		from[i] = flip[(*Main_Pointer & 0b111111000000)>>6]; // Extract "from" position
		to[i]   = flip[*Main_Pointer & 0b111111]; // Extract "to" position
		bits[i] = (*Main_Pointer & 0b111111000000000000)>>12; // Extract "bits"
						//printf("{bits, from, to} octal = {%o, %o, %o}\n", bits, from, to);
						//printf("to = %i\n", to);
						//printf("bits = %i\n\n", bits);
	}
	
	end_HW_to_SW_Timer = *curval;
	
	for (i = 0; i < TotalMoves; ++i) // Pushing moves onto move stack
	{
		if (side == LIGHT)
			gen_push(from[i], to[i], bits[i]);
		else 
			gen_push(flip_horiz[flip[from[i]]], flip_horiz[flip[to[i]]], bits[i]);
	}
	//============== END OUR CODE ============
...
void rawgen()
{
	...
	for (i = 0; i < 64; ++i) // ***START COMMENTING OUT THIS FOR LOOP (when replacing gen())
		if (color[i] == side) {
			if (piece[i] == PAWN) {
				if (side == LIGHT) {
					if (COL(i) != 0 && color[i - 9] == DARK)
...
BOOL makemove(move_bytes m)
{
	...
	if (m.bits & 2) {
		...
		color[to] = color[from]; // THIS IF BLOCK IS JUST FOR CASTLING
		piece[to] = piece[from];
		color[from] = EMPTY;
		piece[from] = EMPTY;
	}
	...
	if (m.bits & 17) // If move was a pawn move or a capture then reset 50 move draw variable
		fifty = 0;
	else
		++fifty;
	...
	side ^= 1; // They are NOTing side
	xside ^= 1;
	if (in_check(xside)) {
		takeback();
		return FALSE;
	}
	set_hash(); // Not needed for our engine
	return TRUE;
}

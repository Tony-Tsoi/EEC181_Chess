...
//================ MY CODE =======================
			...
			for (i = 0; i<64; i++)
			{
				...
				if (i%8 == 7)
				{
					test_module = (int*) get_virtual_addr(0xC0000000, 0x0, j);
					*test_module = current_word;
					current_word = 0;
					j++;
				}
			}
			...
			*test_module = *test_module | 0b1; // Start = 1
			while ( (*test_module & 0b10) == 0b00 ){;} // while(done == 0)
			*test_module = *test_module & (~0x00000001); // Start = 0
			
			test_module = (int*) get_virtual_addr(0xC0000000, 0x0, 10); // HW to SW
			from = flip[(*test_module & 0b111111000000)>>6]; // Extract "from" position
			to   = flip[*test_module & 0b111111]; // Extract "to" position
			...
			// Not 100% sure about this part. If 0 I think there are no more moves.
			// If it's an invalid move then u = 0.
			if (!gen_dat[m].m.u) { 
				computer_side = EMPTY;
